...
openlpChannel.onmessage = function (ev) {
  ...
  if (type === "init") {
    openlpChannel.postMessage(
      JSON.stringify({
        type: "hideOnBlank",
        value: $("#auto-hide-checkbox").prop("checked"),
      })
    );
    openlpChannel.postMessage(
      JSON.stringify({ type: "crossfadeDuration", value: crossfadeDuration })
    );
    openlpChannel.postMessage(
      JSON.stringify({ type: "fadeDuration", value: fadeDuration })
    );
    openlpChannel.postMessage(JSON.stringify({ type: "hide", value: hiding }));
    openlpChannel.postMessage(
      JSON.stringify({ type: "resize", value: autoResize })
    );
    openlpChannel.postMessage(
      JSON.stringify({
        type: "lyricsFont",
        value: $("#lyrics-font-size-spinner").val(),
      })
    );
    openlpChannel.postMessage(
      JSON.stringify({
        type: "titleFont",
        value: $("#title-font-size-spinner").val(),
      })
    );
    openlpChannel.postMessage(
      JSON.stringify({ type: "textFormatting", value: textFormatting })
    );
    openlpChannel.postMessage(
      JSON.stringify({
        type: "maxWidth",
        value: $("#lyrics-max-width-spinner").val(),
      })
    );
    openlpChannel.postMessage(
      JSON.stringify({
        type: "lyricsHeight",
        value: $("#lyrics-height-spinner").val(),
      })
    );
    openlpChannel.postMessage(
      JSON.stringify({
    ...
    if ($("#auto-update-checkbox").prop("checked")) {
      if ($("#display-all-checkbox").prop("checked")) {
        ...
        const defaultValue = Number($("#increment-spinner-1").val());
        displayNext(displayLineCount || defaultValue);
        displayLineCount = 0;
      }
    } else {
      let lines = "";
...
function splitLines(text) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (
      lineWords.length > 1 &&
      lineWordsLength + 1 + wordLength > maxCharacters
    ) {
  ...
  for (i = lines.length - 1; i > 1 && shifted; i--) {
    ...
    while (lines[i].length < minWords) {
      ...
      if (
        lines[i].join(" ").replace(/<[^>]+>/g, "").length +
          newWord.replace(/<[^>]+>/g, "").length +
          1 <
        maxCharacters
      ) {
...
function displayNext(amount) {
  ...
  if (linesToDisplay.length <= 0) {
    displayLineCount = amount;
    displayNextSlide();
    return;
  }
  openlpChannel.postMessage(
    JSON.stringify({ type: "lyrics", value: linesToDisplay })
  );
...
function displaySaved(index) {
  if (index < 0 || index > max_saved_lines || pastLines[index] === undefined) {
    return;
  }
  openlpChannel.postMessage(
    JSON.stringify({ type: "lyrics", value: pastLines[index] })
  );
...
function loadSettings() {
  ...
  let loadedCrossfadeDuration =
    window.localStorage.getItem("crossfadeDuration");
  ...
  let loadedAutoSplitLongLines =
    window.localStorage.getItem("autoSplitLongLines");
  if (loadedAutoSplitLongLines !== null) {
    autoSplitLongLines = loadedAutoSplitLongLines;
    $("#auto-split-long-lines-checkbox").prop(
      "checked",
      loadedAutoSplitLongLines === "true"
    );
  ...
  let loadedTextFormattingAll =
    window.localStorage.getItem("textFormattingAll");
  ...
  let loadedTextFormattingBold =
    window.localStorage.getItem("textFormattingBold");
  ...
  let loadedTextFormattingItalics = window.localStorage.getItem(
    "textFormattingItalics"
  );
  if (loadedTextFormattingItalics !== null) {
    textFormatting["italics"] = loadedTextFormattingItalics === "true";
  }
  $("#text-formatting-italics-checkbox").prop(
    "checked",
    textFormatting["italics"]
  );
  let loadedTextFormattingUnderline = window.localStorage.getItem(
    "textFormattingUnderline"
  );
  if (loadedTextFormattingUnderline !== null) {
    textFormatting["underline"] = loadedTextFormattingUnderline === "true";
  }
  $("#text-formatting-underline-checkbox").prop(
    "checked",
    textFormatting["underline"]
  );
  let loadedTextFormattingColors = window.localStorage.getItem(
    "textFormattingColors"
  );
  if (loadedTextFormattingColors !== null) {
    textFormatting["colors"] = loadedTextFormattingColors === "true";
  }
  $("#text-formatting-colors-checkbox").prop(
    "checked",
    textFormatting["colors"]
  );
  let loadedTextFormattingSuperscript = window.localStorage.getItem(
    "textFormattingSuperscript"
  );
  if (loadedTextFormattingSuperscript !== null) {
    textFormatting["superscript"] = loadedTextFormattingSuperscript === "true";
  }
  $("#text-formatting-superscript-checkbox").prop(
    "checked",
    textFormatting["superscript"]
  );
  let loadedTextFormattingSubscript = window.localStorage.getItem(
    "textFormattingSubscript"
  );
  if (loadedTextFormattingSubscript !== null) {
    textFormatting["subscript"] = loadedTextFormattingSubscript === "true";
  }
  $("#text-formatting-subscript-checkbox").prop(
    "checked",
    textFormatting["subscript"]
  );
  let loadedTextFormattingParagraph = window.localStorage.getItem(
    "textFormattingParagraph"
  );
  if (loadedTextFormattingParagraph !== null) {
    textFormatting["paragraph"] = loadedTextFormattingParagraph === "true";
  }
  $("#text-formatting-paragraph-checkbox").prop(
    "checked",
    textFormatting["paragraph"]
  );
  ...
  let titleVisibleScripture = window.localStorage.getItem(
    "titleVisibleScripture"
  );
  if (titleVisibleScripture !== null) {
    $("#title-visibility-scripture").prop(
      "checked",
      titleVisibleScripture === "true"
    );
  }
...
$(function () {
  ...
  $("#lyrics-max-width-spinner").change(function () {
    ...
    openlpChannel.postMessage(
      JSON.stringify({ type: "maxWidth", value: width })
    );
  });
  $("#lyrics-height-spinner").change(function () {
    ...
    openlpChannel.postMessage(
      JSON.stringify({ type: "lyricsHeight", value: height })
    );
  });
  ...
  $("#auto-hide-checkbox").change(function () {
    ...
    openlpChannel.postMessage(
      JSON.stringify({ type: "hideOnBlank", value: checked })
    );
  });
  ...
  $("#title-visibility-song").change(function () {
    ...
    openlpChannel.postMessage(
      JSON.stringify({ type: "titleVisibility", song: checked })
    );
  });
  $("#title-visibility-scripture").change(function () {
    ...
    openlpChannel.postMessage(
      JSON.stringify({ type: "titleVisibility", bible: checked })
    );
  });
  $("#lyrics-font-size-spinner").change(function () {
    ...
    openlpChannel.postMessage(
      JSON.stringify({ type: "lyricsFont", value: font })
    );
  });
  $("#title-font-size-spinner").change(function () {
    ...
    openlpChannel.postMessage(
      JSON.stringify({ type: "titleFont", value: font })
    );
  });
...
const controlChannel = new BroadcastChannel("obs_openlp_control_channel");
controlChannel.addEventListener("message", (event) => {
  const data = JSON.parse(event.data);

  if (data.type === "displayNext") {
    displayNext(data.value);
  } else if (data.type === "undoDisplay") {
    displaySaved(historyIndex + 1);
  } else if (data.type === "redoDisplay") {
    displaySaved(historyIndex - 1);
  } else if (data.type === "nextSlide") {
    displayNextSlide();
  } else if (data.type === "prevSlide") {
