...
window.OpenLP = {
  ...
  loadSlides: function (event) {
    $.getJSON("/api/controller/live/text", function (data, status) {
  ...
  updateText: function () {
    ...
    obsChannel.postMessage(
      JSON.stringify({ type: "lyrics", lines: text.split(/\n/g) })
    );
  },
  pollServer: function () {
  ...
  channelReceive: function (ev) {
    ...
    if (updateLayout) {
      ...
      if (autoResize) {
        ...
        while (
          lyricsContainer.outerHeight() > lyricsParent.innerHeight() &&
          lyricsParent.innerHeight() > 0
        ) {
  ...
  filterTags: function (string, allowedTags) {
    ...
    $.each(allowedTags, function (idx, longTag) {
      ...
      string = string.replace(
        new RegExp("<" + longTag + "([^>]*)>", "gi"),
        "[" + longTag + "$1]"
      );
      string = string.replace(
        new RegExp("</" + onlyTag + ">", "gi"),
        "[/" + onlyTag + "]"
      );
    });
